...
class ProductFilterTests(TestCase):
    ...
    def setUp(self):
        ...
        # SE CORRIGIÓ: Precio de 50 a 500 para que no interfiera con el test de precio máximo
        create_product('Inactive', price=500, stock=10, category=cat, is_active=False)

    def test_filter_by_max_price(self):
        resp = self.client.get('/api/products/?price_max=100')
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        # Ahora solo encontrará 1 producto ('Cheap') porque 'Inactive' cuesta 500
