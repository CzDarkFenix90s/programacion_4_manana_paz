// lib/screens/pantalla_dialogs_mp_paz.dart
...
class PantallaDialogs extends StatelessWidget {
    ...
    showDialog<void>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text(
          ...
          'Por favor, guarde todos sus registros de plan antes de esa hora.',
        ),
        actions: [
          TextButton(
  ...
  Future<void> _mostrarConfirmacion(BuildContext context) async {
    final confirmar = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text(
          '¿Estás seguro de que deseas eliminar Paciente Juan?\n'
    ...
    if (confirmar == true) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Bus eliminado correctamente')),
      );
    }
  }
  ...
  Future<void> _mostrarFormulario(BuildContext context) async {
    ...
    if (ctrlNombre.text.isNotEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Bus "${ctrlNombre.text}" agregado')),
      );
    }
  }
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          // ── Sección AlertDialog / Observaciones ──────────────────────────
          Text('Observaciones y Diálogos', style: text.labelLarge?.copyWith(color: cs.primary)),
          const SizedBox(height: 12),
          ...
          OutlinedButton.icon(
            ...
            label: const Text('Eliminar bus (Confirmación)'),
          ),
          const SizedBox(height: 8),
